...
def test_task3_detailed():
    """
    Detailed test for Task 3: OpenAPI to FastAPI converter
    Expected: Master should delegate to Researcher, BackendDev, Tester, DocWriter, DevOps
    """
    load_dotenv()
    
    session_id = "task3_detailed"
    user_request = """Создай утилиту для конвертации REST API-спецификации OpenAPI в полностью рабочий FastAPI-проект с валидацией и тестами.
    Мастер делегирует Researcher'у изучить структуру OpenAPI и FastAPI-совместимость, BackendDev'у — генератор кода из YAML/JSON, Tester'у — автогенерацию тестов для всех эндпоинтов, DocWriter'у — README и примеры запуска, DevOpsAgent'у — Dockerfile и docker-compose.yml."""
    
    print(f"=== TESTING TASK 3: OpenAPI to FastAPI Converter ===")
